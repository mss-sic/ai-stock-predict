import json, time, psycopg2, urllib.request, ssl
...
for i, code in enumerate(codes):
    ...
    try:
        ...
        if rows:
            execute_values(cur, """INSERT INTO stocks_daily_k (code,trade_date,open,high,low,close,volume,amount)
                VALUES %s ON CONFLICT (code,trade_date) DO NOTHING""", rows)
    ...
    if (i + 1) % 500 == 0:
        conn.commit()
        print(f"[{i+1}/{total_stocks}] {total_inserted} records", flush=True)

conn.commit()
cur.execute("SELECT COUNT(*), MIN(trade_date), MAX(trade_date) FROM stocks_daily_k")
c, mn, mx = cur.fetchone()
print(f"DONE: {c} total records, {mn} ~ {mx}", flush=True)
