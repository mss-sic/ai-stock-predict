...
type StockFinancial struct {
	...
	// v099: Cash Flow Statement (现金流量表) — in 万元
	OperatingCF float64 `json:"operatingCf"` // 经营活动现金流净额
	InvestingCF float64 `json:"investingCf"` // 投资活动现金流净额
	FinancingCF float64 `json:"financingCf"` // 筹资活动现金流净额
	NetCashFlow float64 `json:"netCashFlow"` // 现金及等价物净增加额
	FreeCF      float64 `json:"freeCf"`      // 自由现金流(经营+投资)
	CFRatio     float64 `json:"cfRatio"`     // 现金流/净利润比%
