...
type LegacyRiskRule struct {
	Type        string
	Level       string
	Description string
}

// codesToInClause builds a SQL IN clause string from stock codes.
...
// ScanUserHoldings runs the new RiskEngine full scan (replaces old logic).
// Kept for backward compat with existing callers (handler, scheduler, task_service).
func ScanUserHoldings() (int, error) {
	return GetEngine().ScanAll(context.Background())
}

// GetUserRiskAlerts returns active (non-ignored, non-resolved) risk alerts for user's holdings.
// Backward compat: sets Ignored=true on the model when status='ignored'.
func GetUserRiskAlerts(userID uint) ([]model.RiskAlert, error) {
	...
	db.MySQL.Where("(user_id = ? AND status = 'active') OR (user_id = 0 AND stock_code = '__MARKET__' AND status = 'active')", userID).
	...
	if len(alerts) > 0 {
		...
		for i := range alerts {
			...
			// Backward compat: set Ignored from status
			if alerts[i].Status == "ignored" {
				alerts[i].Ignored = true
			}
		}
	}
...
// IgnoreRiskAlert marks an alert as ignored (dual-write: sets both status and ignored flag).
func IgnoreRiskAlert(userID, alertID uint) error {
	...
	return db.MySQL.Model(&model.RiskAlert{}).
		...
		Updates(map[string]interface{}{
			...
			"status":         "ignored",
			"resolved_at":    now,
			"resolution_note": "用户手动忽略",
		}).Error
}

// ScanUserHoldingsLegacy is the old scan logic preserved for reference during migration.
// It will be removed after Phase 4 parallel validation.
func ScanUserHoldingsLegacy() (int, error) {
	...
	type KlineChg struct{ Code string; ChgPct float64 }
	var drops []KlineChg
	...
	type MACross struct{ Code string; Close, MA20, PrevC, PrevM float64 }
	var crosses []MACross
	db.PG.Raw(fmt.Sprintf(`
		WITH ranked AS (
	...
	type PEInfo struct{ Code string; PE, IndAvg float64 }
	var peInfos []PEInfo
	db.PG.Raw(fmt.Sprintf(`
		WITH latest_pe AS (
			SELECT DISTINCT ON (code) code, pe FROM stocks_daily_indicator
			WHERE code IN (%s) AND pe > 0 ORDER BY code, trade_date DESC
		),
		ind_avg AS (
	...
	db.PG.Raw(fmt.Sprintf(`
		...
		FROM latest WHERE profit_growth < -30
	...
	log.Printf("[RiskService-legacy] scan complete: %d alerts for %d stocks from %d holdings",
...
// GenerateRiskSnapshot creates a daily risk snapshot for trend tracking.
func GenerateRiskSnapshot() error {
	// Count alerts by level
	var high, medium, low int64
	db.MySQL.Model(&model.RiskAlert{}).Where("level = 'high' AND status = 'active' AND DATE(hit_date) = CURRENT_DATE").Count(&high)
	db.MySQL.Model(&model.RiskAlert{}).Where("level = 'medium' AND status = 'active' AND DATE(hit_date) = CURRENT_DATE").Count(&medium)
	db.MySQL.Model(&model.RiskAlert{}).Where("level = 'low' AND status = 'active' AND DATE(hit_date) = CURRENT_DATE").Count(&low)
	...
	var dims []DimCount
	db.MySQL.Raw("SELECT COALESCE(dimension,'stock') as dimension, COUNT(*) as cnt FROM risk_alerts WHERE status='active' AND DATE(hit_date)=CURRENT_DATE GROUP BY dimension").Scan(&dims)
	bd := make(model.JSONMap)
	for _, d := range dims {
		bd[d.Dimension] = float64(d.Cnt)
	}

	// Market risk level heuristic
	marketLevel := "low"
	marketScore := 0
	if high > 3 || (bd["market"] != nil && bd["market"].(float64) >= 2) {
		marketLevel = "critical"
		marketScore = 80
	} else if high > 0 {
		marketLevel = "high"
		marketScore = 60
	...
	snap := model.RiskSnapshot{
		...
		MarketRiskLevel:    marketLevel,
		MarketRiskScore:    marketScore,
		TotalAlertsHigh:    int(high),
		TotalAlertsMedium:  int(medium),
		TotalAlertsLow:     int(low),
		DimensionBreakdown: bd,
	}
	return db.MySQL.Create(&snap).Error
}

// ScanRiskEvents scans only event-type rules (E1-E3) for pre-market.
func ScanRiskEvents() (int, error) {
	engine := GetEngine()
	return engine.ScanDimension("event")
}
