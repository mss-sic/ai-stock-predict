...
- 新增统一 K 线写入器，按数据源优先级保护高质量行情，统一维护 `data_source`、`source_priority`、`data_quality` 与复权因子。
- 腾讯与 Tushare 采集接入统一写入链路，保留昨收价、涨跌额、成交量和成交额单位语义。
- 新增历史 K 线回填、复权因子计算、交易日历同步、数据质量扫描与校验工具。
- 新增数据仪表盘，展示 K 线覆盖、异常/稀疏股票和财务现金流覆盖情况。

### 技术指标与行情展示

- 新增 `stock_daily_indicators` JSONB 指标缓存及批量计算任务，盘后流水线自动预计算技术指标。
- 指标计算改为批量加载估值数据、批量 UPSERT，失败时回滚并向调度器返回错误，避免 N+1 和假成功。
- 新增交易日历、价格与指标缓存 Repository/Service，优化全市场指标扫描。
- 个股详情页新增指标表，K 线图补充复权与 MACD 数据展示。

### 财务与预测能力

- 财务数据增加经营、投资、筹资、净现金流、自由现金流及现金流质量指标，并提供历史回填脚本。
- 新增 `prediction_factors` 预计算表，预测精选由运行时 JSON 解析改为直接查询因子表。
- 预测导入同步批量生成 5/10/20 日一致性、预期收益、动量和波动因子；迁移自动回填既有预测数据。

### 数据库与稳定性

- 新增 v097-v101 迁移及对应线上修复 SQL，覆盖交易日历、行情元数据、现金流字段、指标缓存和预测因子。
- 修复股东采集脚本缩进错误、交易日历字段名错误、预测因子批量参数错误。
- 修复调度 Worker timeout context 延迟释放、采集进度复制锁以及统计接口吞掉数据库错误的问题。
- 清理不对应生产实现的孤立编排测试，并修正按秒 Cron 测试的时间假设。
...
- **置信度=0 Bug**: `ComputeAndStore` 重复 roll 查询无错误处理，静默失败时 roll 归零致 rotation/bull_rally/mild_bull 置信度系统性为 0。提取 `computeRollingStats()` 消除重复查询，增加错误检查，批量重算 66 条历史记录。
- **style_duration 修复**: 批量重算导致 duration 链断裂，用顺序重算恢复。当前 rotation 已持续 6 天。
- **transition_signal 优化**: reversal 阈值从 2→3 次风格切换，减少过度标记。
- **前端优化**: 菜单「市场复盘」→「市场风格」，DatePicker 禁止选今天及未来日期，数据默认取 T-1。

### 基础数据采集修复

- **北交所 K 线采集**: 腾讯 API 使用 `nq` 前缀，新增 92xxx/8xxx 前缀判断。修复前缀顺序（92 优先于 9）避免误判为上交所。API 返回键名自动纠正时多前缀回退。
- **换手率修复**: 从依赖实时行情 API 改为 `circulating_market_cap / close` 计算流通股本 → 换手率 = `volume / 流通股本`，覆盖 4856 只股票。实时行情保留为 PE/市值补充。
- **SQL 排除修复**: 移除 `NOT LIKE '88%'` 排除条件，避免误排除北交所股票。
- **错误处理增强**: `fetch_quote_batch` 从静默 `except: pass` 改为显式日志记录。
- **已知限制**: 83/87 系列老北交所股票腾讯数据止于 2025-04~09，需后续补充其他数据源。

## v1.8.3 (2026-07-02)

### K线采集重构

- **腾讯 qt 全字段提取**: 单次请求同时获取 88 列实时行情（换手率/PE/PB/市值/涨跌停/外盘内盘/涨跌幅/振幅/量比），替代分阶段采集
- **数据单位标准化**: volume=股、amount=元、turnoverRate=原始比率，统一不同数据源输出格式
- **新增入库字段**: buy_vol（外盘）、sell_vol（内盘）、change_pct（涨跌幅%）、amplitude（振幅%）、volume_ratio（量比）
- **采集优化**: 去掉过期判断，始终拉取最近 10 天；交易日每 30 分钟自动采集

### 换手率修复

- **单位对齐历史数据**: turnover_rate 改为 `qt[38]/100` 原始比率格式，与前端 ×100 显示一致
- **UPSERT 保护**: `CASE WHEN >0` 防止非最新日期行覆盖历史有效值为 0

### PE/PB 采集清理

- 删除 `daily_indicator.py` / `backfill_indicator.py` 独立采集脚本
- 移除 engine 和 task_service 中 PE/PB 采集阶段和定时任务
- PE/PB/市值现由 batch_collect 腾讯 qt 阶段统一入库
- 废弃定时任务启动时自动禁用

### 新字段整合 & 显示修复

- **StockListPage**: 修复换手率缺少 ×100 导致 0.5% 显示为 0.01% 的 bug
- **StockDetailPage**: 振幅改用存储字段（且修正原代码用 open 作分母的非标准算法）
- **stock_repo.go**: 振幅 SQL 改用 `k.amplitude` 直接读取
- **industry_service.go**: 涨跌幅改用 `k.change_pct` 直接读取，减少 LATERAL 子查询

### 资金流向新指标

- 新增 `net_flow_ratio`（资金净流比）: `(外盘-内盘)/成交量×100`，>0 主动买入占优
- 新增 `buy_sell_ratio`（内外盘比）: `外盘/内盘`，>1 主动买入明显
- 每日采集自动积累历史，30 交易日后可回测

### 市场风格 & 采集修复

- 涨跌停统计改用涨停价精确匹配，对齐东方财富口径
- 市场风格采集始终覆盖最新日期，防止脏数据残留
- RunManualCollection 空切片不再触发全量采集
- 采集控制台选择市场风格时不再误触发其他采集
