...
export default function ConceptBoardDetailPage() {
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-3)' }}>加载中...</div>;
  }
  if (!data) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-3)' }}>板块不存在</div>;
  }
      ...
      <div style={{ background: 'var(--color-bg-1)', borderRadius: 8, border: '1px solid var(--color-border-1)', overflow: 'hidden' }}>
        ...
        <div style={{ overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--color-fill-2)', color: 'var(--color-text-3)', fontSize: 11 }}>
            ...
            <tbody>
              {(stocks || []).map((s: any, i: number) => (
                <tr key={i} onClick={() => navigate(`/stock/${s.code}`)} style={{
                  ...
                  <td style={{ padding: '8px 12px', fontFamily: 'monospace', color: 'var(--color-text-3)', fontSize: 12 }}>{s.code}</td>
                  ...
                  <td style={{ padding: '8px 12px', textAlign: 'right', color: 'var(--color-text-3)', fontSize: 12 }}>
